...
def instalar(app):
    if getattr(app, '_seven_payment_v06', False): return
    app._seven_payment_v06 = True

    def db(): return app.config['SEVEN_DB']()
    ...
    def mp_request(method,path,payload=None,idem=None):
        if not token(): raise RuntimeError('MERCADOPAGO_ACCESS_TOKEN não configurado no Render.')
    ...
    def criar_pix(venda_id,total,descricao,email):
        ...
        try:
            with conn.cursor() as cur:
                cur.execute("UPDATE vendas SET pagamento_id=%s,pagamento_status=%s,pagamento_qr=%s,pagamento_copia_cola=%s,pagamento_criado_em=CURRENT_TIMESTAMP,status='aguardando_pagamento' WHERE id=%s",(str(p.get('id')),p.get('status','pending'),tx.get('qr_code_base64'),tx.get('qr_code'),venda_id))
            conn.commit()
        finally: conn.close()
        return p
    ...
    def interceptar_pix_v06():
        if request.endpoint!='vendas' or request.method!='POST' or request.form.get('pagamento','').strip().lower()!='pix': return None
        try:
        ...
        if '@' not in email: return render_template_string(EMAIL_PAGE)
        uid=app.view_functions['usuario_id_atual'](); caixa=app.view_functions['garantir_caixa'](uid); descricao=request.form.get('descricao','').strip()
        conn=db()
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT pix_chave FROM configuracoes_loja WHERE id=1"); loja=cur.fetchone() or {}
                if not loja.get('pix_chave'): return redirect('/vendas?erro=Chave+Pix+nao+cadastrada')
        ...
        try: p=criar_pix(venda_id,total,descricao,email)
        ...
        tx=(p.get('point_of_interaction') or {}).get('transaction_data') or {}
        return render_template_string(PAYMENT_PAGE,venda_id=venda_id,total=f'{total:.2f}',qr=tx.get('qr_code_base64',''),copia=tx.get('qr_code',''))

    def confirmar(venda_id,payment_id,payment):
        conn=db()
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT total,status,pagamento_id FROM vendas WHERE id=%s FOR UPDATE",(venda_id,)); v=cur.fetchone()
                if not v or v['status']!='aguardando_pagamento' or str(v['pagamento_id'])!=str(payment_id): return False
                recebido=Decimal(str(payment.get('transaction_amount') or '0'))
                esperado=Decimal(str(v['total']))
                if payment.get('status')!='approved' or recebido!=esperado or str(payment.get('external_reference') or '')!=str(venda_id):
                    return False
    ...
    def status_pagamento_v06(venda_id):
        ...
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT id,total,pagamento,status,pagamento_id,pagamento_status FROM vendas WHERE id=%s AND usuario_id=%s",(venda_id,app.view_functions['usuario_id_atual']())); v=cur.fetchone()
        finally: conn.close()
        if not v: return {'erro':'Venda não encontrada'},404
        if v['pagamento_id']:
            try:
                p=mp_request('GET','/v1/payments/'+str(v['pagamento_id'])); st=p.get('status','pending')
                if st=='approved': confirmar(venda_id,v['pagamento_id'],p)
    ...
    def webhook_mercadopago_v06():
        data=request.get_json(silent=True) or {}; pid=data.get('data',{}).get('id') or request.args.get('id'); typ=data.get('type') or request.args.get('topic')
        if typ not in ('payment','payments') or not pid: return 'ok',200
        try:
            p=mp_request('GET','/v1/payments/'+str(pid)); ref=p.get('external_reference')
            if ref and p.get('status')=='approved': confirmar(int(ref),str(pid),p)
            elif ref and p.get('status') in ('rejected','cancelled','refunded','charged_back'): marcar_falha(int(ref),p.get('status'))
        except Exception: pass
        return 'ok',200

EMAIL_PAGE='''<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Pix - Seven Store</title><style>body{font-family:Arial;max-width:500px;margin:50px auto;padding:20px}.box{padding:25px;border-radius:15px;box-shadow:0 5px 20px #ddd}input,button{width:100%;padding:13px;margin-top:10px;box-sizing:border-box}button{cursor:pointer}</style><div class="box"><h2>💳 Pagamento Pix</h2><p>Informe o e-mail do cliente para criar a cobrança Pix.</p><form method="post" action="/vendas"><input type="hidden" name="total" value="{{ request.form.get('total','') }}"><input type="hidden" name="pagamento" value="pix"><input type="hidden" name="descricao" value="{{ request.form.get('descricao','') }}"><input name="email_cliente" type="email" required placeholder="E-mail do cliente"><button>Gerar cobrança Pix</button></form></div>'''
