...
def instalar(app):
    ...
    def listar_vendas_pendentes_compartilhadas():
        if not autorizado(): return jsonify({"erro": "Faça login novamente."}), 401
        caixa = app.garantir_caixa(uid()); conexao = banco()
        try:
            with conexao.cursor() as cursor:
        ...
        for row in rows:
            item=dict(row)
            if isinstance(item.get("total"),Decimal): item["total"]=float(item["total"])
            if isinstance(item.get("criada_em"),datetime): item["criada_em"]=item["criada_em"].strftime("%d/%m/%Y %H:%M")
    ...
    def criar_venda_pendente_compartilhada():
        ...
        except (InvalidOperation,ValueError,TypeError): return jsonify({"erro":"Informe um valor válido."}),400
        pagamento=str(dados.get("pagamento","dinheiro")).strip() or "dinheiro"; descricao=str(dados.get("descricao","")).strip()[:255]
        caixa=app.garantir_caixa(uid()); conexao=banco()
        try:
            with conexao.cursor() as cursor:
                cursor.execute("""INSERT INTO vendas_pendentes(usuario_id,caixa_id,total,pagamento,descricao) VALUES(%s,%s,%s,%s,%s)
                    RETURNING id,total,pagamento,descricao,criada_em""",(uid(),caixa["id"],total,pagamento,descricao));row=cursor.fetchone()
            conexao.commit()
        finally: conexao.close()
        item=dict(row);item["total"]=float(item["total"])
        if isinstance(item.get("criada_em"),datetime):item["criada_em"]=item["criada_em"].strftime("%d/%m/%Y %H:%M")
    ...
    def continuar_venda_pendente_compartilhada(pendente_id):
        ...
        try:
            with conexao.cursor() as cursor:
                cursor.execute("SELECT id,total,pagamento,descricao FROM vendas_pendentes WHERE id=%s AND caixa_id=%s",(pendente_id,caixa["id"]));row=cursor.fetchone()
                if not row:return jsonify({"erro":"Venda pendente não encontrada neste caixa."}),404
                cursor.execute("DELETE FROM vendas_pendentes WHERE id=%s AND caixa_id=%s",(pendente_id,caixa["id"]))
            conexao.commit()
        finally:conexao.close()
        item=dict(row);item["total"]=float(item["total"]);return jsonify({"ok":True,"pendente":item})
    ...
    def excluir_venda_pendente_compartilhada(pendente_id):
        ...
        caixa=app.garantir_caixa(uid());conexao=banco()
        try:
            with conexao.cursor() as cursor:cursor.execute("DELETE FROM vendas_pendentes WHERE id=%s AND caixa_id=%s",(pendente_id,caixa["id"]));apagadas=cursor.rowcount
            conexao.commit()
        finally:conexao.close()
        if not apagadas:return jsonify({"erro":"Venda pendente não encontrada neste caixa."}),404
    ...
    def buscar_integracoes(conexao):
        with conexao.cursor() as cursor:
            cursor.execute("SELECT canal,account_id,page_id,phone_number_id,business_account_id,ativo FROM integracoes WHERE usuario_id=%s ORDER BY canal",(uid(),))
            return cursor.fetchall()

    def atendimento_view():
        if not autorizado(): return redirect("/")
        if not perfil_permitido(): return redirect("/inicio")
        conversa_id=request.args.get("conversa",type=int);conexao=banco()
        try:
            conversas=buscar_conversas(conexao);atual=None;mensagens=[]
            with conexao.cursor() as cursor:
                if conversa_id:
        ...
        for i in integracoes:
            if i["canal"] in estados and i["ativo"]: estados[i["canal"]]="Conectado"
        return render_template("atendimento.html",**app.contexto_base(),conversas=conversas,conversa_atual=atual,mensagens=mensagens,conversa_selecionada=conversa_id,whatsapp_status=estados["whatsapp"],instagram_status=estados["instagram"],integracoes=integracoes)

    app.view_functions["atendimento"]=atendimento_view

    @app.route("/configuracoes/integracao",methods=["POST"])
    def salvar_integracao():
        if not autorizado() or not app.exigir_perfil("administrador"): return redirect("/inicio")
        canal=request.form.get("canal","").strip().lower()
        if canal not in ("whatsapp","instagram"): return redirect("/configuracoes")
        account_id=request.form.get("account_id","").strip();page_id=request.form.get("page_id","").strip();phone_id=request.form.get("phone_number_id","").strip();business_id=request.form.get("business_account_id","").strip();token=request.form.get("access_token","").strip();verify=request.form.get("verify_token","").strip();ativo=bool(token)
        conexao=banco()
        try:
            with conexao.cursor() as cursor:
                cursor.execute("""INSERT INTO integracoes(usuario_id,canal,account_id,page_id,phone_number_id,business_account_id,access_token,verify_token,ativo)
                    ...
                    ON CONFLICT(usuario_id,canal) DO UPDATE SET account_id=EXCLUDED.account_id,page_id=EXCLUDED.page_id,phone_number_id=EXCLUDED.phone_number_id,business_account_id=EXCLUDED.business_account_id,access_token=EXCLUDED.access_token,verify_token=EXCLUDED.verify_token,ativo=EXCLUDED.ativo""",(uid(),canal,account_id,page_id,phone_id,business_id,token,verify,ativo))
            conexao.commit()
        finally:conexao.close()
    ...
    def enviar_mensagem(conversa_id):
        if not autorizado() or not perfil_permitido():return redirect("/inicio")
        texto=request.form.get("mensagem","").strip()
        if not texto:return redirect(url_for("atendimento",conversa=conversa_id))
        conexao=banco()
        try:
            with conexao.cursor() as cursor:
                cursor.execute("SELECT * FROM conversas WHERE id=%s",(conversa_id,));conversa=cursor.fetchone()
                if not conversa:return redirect("/atendimento")
        ...
        try:enviar_meta(conversa["canal"],conversa["identificador"],texto)
    ...
    def enviar_meta(canal,recipient,texto):
        ...
        if not integ or not integ["access_token"]:return False
    ...
    def salvar_mensagem_webhook(canal,identificador,nome,texto,externa_id=None):
        ...
        try:
            with conexao.cursor() as cursor:
                cursor.execute("SELECT id FROM usuarios WHERE perfil='administrador' ORDER BY id LIMIT 1");owner=cursor.fetchone()
                if not owner:return
                cursor.execute("SELECT id FROM conversas WHERE canal=%s AND identificador=%s",(canal,identificador));conv=cursor.fetchone()
                if conv:cid=conv["id"]
                else:
                    cursor.execute("INSERT INTO conversas(usuario_id,canal,identificador,nome,ultima_mensagem) VALUES(%s,%s,%s,%s,%s) RETURNING id",(owner["id"],canal,identificador,nome,texto));cid=cursor.fetchone()["id"]
    ...
    def webhook_meta():
        if request.method=="GET":
            ...
            if mode=="subscribe":
                ...
                try:
                    with conexao.cursor() as cursor:
                        cursor.execute("SELECT verify_token FROM integracoes WHERE verify_token=%s AND ativo=TRUE LIMIT 1",(token,));ok=cursor.fetchone()
                finally:conexao.close()
                if ok:return challenge or "",200
            return "Verificação inválida",403
        data=request.get_json(silent=True) or {}
        for entry in data.get("entry",[]):
            for change in entry.get("changes",[]):
                value=change.get("value",{});canal="whatsapp" if "messages" in value else "instagram"
                ...
                for msg in value.get("messaging",[]):
                    ...
                    if texto:salvar_mensagem_webhook("instagram",msg.get("sender",{}).get("id",""),None,texto,msg.get("message",{}).get("mid"))
