...
def conectar_banco():
    ...
    if not database_url: raise RuntimeError("DATABASE_URL não configurada no Render.")
...
def criar_banco():
    ...
    try:
        with banco.cursor() as cursor:
            ...
            cursor.execute("CREATE TABLE IF NOT EXISTS vendas (id SERIAL PRIMARY KEY, usuario_id INTEGER REFERENCES usuarios(id) ON DELETE SET NULL, caixa_id INTEGER REFERENCES caixas(id) ON DELETE SET NULL, total NUMERIC(12,2) NOT NULL, pagamento VARCHAR(30) NOT NULL, descricao VARCHAR(255), pix_chave VARCHAR(255), status VARCHAR(20) NOT NULL DEFAULT 'concluida', cancelada_em TIMESTAMP, cancelada_por INTEGER REFERENCES usuarios(id) ON DELETE SET NULL, motivo_cancelamento VARCHAR(255), criada_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
            cursor.execute("ALTER TABLE vendas ADD COLUMN IF NOT EXISTS status VARCHAR(20) NOT NULL DEFAULT 'concluida'")
            cursor.execute("ALTER TABLE vendas ADD COLUMN IF NOT EXISTS cancelada_em TIMESTAMP")
            cursor.execute("ALTER TABLE vendas ADD COLUMN IF NOT EXISTS cancelada_por INTEGER REFERENCES usuarios(id) ON DELETE SET NULL")
            cursor.execute("ALTER TABLE vendas ADD COLUMN IF NOT EXISTS motivo_cancelamento VARCHAR(255)")
            ...
            if cursor.fetchone()["total"]:
                ...
                if cursor.fetchone()["total"]==0: cursor.execute("UPDATE usuarios SET perfil='administrador' WHERE id=(SELECT id FROM usuarios ORDER BY id ASC LIMIT 1)")
...
def usuario_atual():
    ...
    try:
        with banco.cursor() as cursor:
            cursor.execute("SELECT * FROM usuarios WHERE id=%s",(session["usuario_id"],)); return cursor.fetchone()
    finally:banco.close()
...
def fazer_login():
    ...
    banco=conectar_banco()
    try:
        with banco.cursor() as cursor:cursor.execute("SELECT * FROM usuarios WHERE usuario=%s AND senha=%s",(usuario,senha)); resultado=cursor.fetchone()
    finally:banco.close()
    if resultado:
...
def api_relatorios():
    ...
    try:
        with banco.cursor() as cursor:
            cursor.execute("SELECT COALESCE(SUM(total),0) AS total,COUNT(*) AS quantidade FROM vendas WHERE criada_em >= %s AND status='concluida'",(inicio,));resumo=cursor.fetchone()
            cursor.execute("SELECT pagamento,COUNT(*) AS quantidade,COALESCE(SUM(total),0) AS total FROM vendas WHERE criada_em >= %s AND status='concluida' GROUP BY pagamento ORDER BY total DESC",(inicio,));pagamentos=cursor.fetchall()
            cursor.execute("SELECT EXTRACT(HOUR FROM criada_em)::int AS hora,COUNT(*) AS quantidade,COALESCE(SUM(total),0) AS total FROM vendas WHERE criada_em >= %s AND status='concluida' GROUP BY hora ORDER BY hora",(inicio,));horarios=cursor.fetchall()
            cursor.execute("SELECT c.id,c.nome,COALESCE(u.usuario,'Sem usuário') AS usuario,COUNT(v.id) AS quantidade,COALESCE(SUM(v.total),0) AS total FROM caixas c LEFT JOIN usuarios u ON u.id=c.usuario_id LEFT JOIN vendas v ON v.caixa_id=c.id AND v.criada_em >= %s AND v.status='concluida' GROUP BY c.id,c.nome,u.usuario ORDER BY total DESC,quantidade DESC",(inicio,));ranking=cursor.fetchall()
            cursor.execute("SELECT TO_CHAR(criada_em,'DD/MM') AS dia,COUNT(*) AS quantidade,COALESCE(SUM(total),0) AS total FROM vendas WHERE criada_em >= %s AND status='concluida' GROUP BY DATE(criada_em),TO_CHAR(criada_em,'DD/MM') ORDER BY DATE(criada_em)",(inicio,));por_dia=cursor.fetchall()
    finally:banco.close()
    def clean(rows):
        out=[]
        for row in rows:
...
def caixa():
    ...
    try:
        with banco.cursor() as cursor:
            cursor.execute("SELECT COALESCE(SUM(total),0) AS total FROM vendas WHERE caixa_id=%s AND status='concluida'",(meu_caixa["id"],));total=cursor.fetchone()["total"]
...
def vendas():
        ...
        try:
            ...
            try:
                with banco.cursor() as cursor:
                    ...
                    cursor.execute("INSERT INTO vendas (usuario_id,caixa_id,total,pagamento,descricao,pix_chave,status) VALUES (%s,%s,%s,%s,%s,%s,'concluida')",(uid,caixa_atual["id"],total,pagamento,descricao,pix_chave if pagamento=="pix" else None))
                banco.commit()
            finally:banco.close()
    ...
    try:
        with banco.cursor() as cursor:
            ...
            cancelamentos=[]
            if exigir_perfil("administrador","subadministrador"):
                cursor.execute("SELECT v.*,u.usuario AS vendedor,c.nome AS caixa_nome FROM vendas v LEFT JOIN usuarios u ON u.id=v.usuario_id LEFT JOIN caixas c ON c.id=v.caixa_id ORDER BY v.criada_em DESC LIMIT 100");cancelamentos=cursor.fetchall()
    finally:banco.close()
    return render_template("vendas.html",**contexto_base(),caixa=caixa_atual,loja=loja,vendas=vendas_recentes,cancelamentos=cancelamentos,mensagem=mensagem,erro=erro)
@app.route("/vendas/<int:venda_id>/cancelar",methods=["POST"])
def cancelar_venda(venda_id):
    if not exigir_login() or not exigir_perfil("administrador","subadministrador"):return jsonify({"erro":"Apenas Administrador e Sub-administrador podem cancelar vendas."}),403
    motivo=request.form.get("motivo","").strip() or "Cancelamento autorizado"
    banco=conectar_banco()
    try:
        with banco.cursor() as cursor:
            cursor.execute("SELECT id,status FROM vendas WHERE id=%s FOR UPDATE",(venda_id,));venda=cursor.fetchone()
            if not venda:return jsonify({"erro":"Venda não encontrada."}),404
            if venda["status"]=="cancelada":return jsonify({"erro":"Esta venda já está cancelada."}),400
            cursor.execute("UPDATE vendas SET status='cancelada',cancelada_em=CURRENT_TIMESTAMP,cancelada_por=%s,motivo_cancelamento=%s WHERE id=%s",(usuario_id_atual(),motivo,venda_id))
        banco.commit()
    finally:banco.close()
...
def pix_qr():
    ...
    if not chave:
        banco=conectar_banco()
        try:
            with banco.cursor() as cursor:cursor.execute("SELECT pix_chave,pix_nome,pix_cidade FROM configuracoes_loja WHERE id=1");loja=cursor.fetchone() or {}
        finally:banco.close()
...
def configuracoes():
    ...
    mensagem=None;banco=conectar_banco()
    try:
        with banco.cursor() as cursor:
            if request.method=="POST":
                chave=request.form.get("pix_chave","").strip();nome=request.form.get("pix_nome","").strip();cidade=request.form.get("pix_cidade","").strip();cursor.execute("UPDATE configuracoes_loja SET pix_chave=%s,pix_nome=%s,pix_cidade=%s,atualizado_em=CURRENT_TIMESTAMP WHERE id=1",(chave,nome,cidade));banco.commit();mensagem="Configurações salvas com sucesso."
            cursor.execute("SELECT pix_chave,pix_nome,pix_cidade FROM configuracoes_loja WHERE id=1");loja=cursor.fetchone() or {}
